...
$pageTitle = "<strong>" . htmlentities( " " . "INCOME STATEMENT FROM  ") .$_SESSION['datefrom'] ." To ".$_SESSION['dateto']. "</strong>";
...
$pageTitle = "<strong>" . htmlentities( " " . "Incomes " . strtoupper("")) . "</strong>";
    ...
    <tbody>
        <tr>
           ...
           <td style="width: 45%; text-align: left;">' . html_entity_decode(htmlentities($row["acc_name"],ENT_QUOTES),ENT_QUOTES) . '</td>
            ...
            <td style="width: 35%; text-align: left;">' . htmlentities("".number_format( $row["acc_amt"], 2, ".", ",")) . '</td>
...
<td style="text-align: left; width: 65%;"><strong>TOTAL AMOUNT</strong></td>  
 <td style="text-align: left; width: 35%;">'. htmlentities("GHS ".number_format( $searchincomexpensesum, 2, ".", ",")) . '</td>
</tr>
</tbody>';
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
         ...
         <td style="text-align: left; width: 45%;"><strong>Fullname</strong></td>
...
while ($row = mysqli_fetch_array($searchpayements)) {
             ...
             $label = "Payment At ".$row["payment_date"]; 
         $memberdetails =   member_details($row['member_id']);

             $fulname = $memberdetails['last_name'].' '.$memberdetails['first_name'];
             $ranks = $memberdetails['ranks'];
             $tittles = $memberdetails['tittles'];
    ...
    <tbody>
        <tr>
            ...
            <td style="text-align: left; width: 45%;">' . $formatted . '</td>
            
           <td style="width: 25%; text-align: left;">' . htmlentities($label) . '</td>
   ...
   <tr>
<td style="text-align: left;width: 75%;"><strong>TOTAL AMOUNT</strong></td>   
 <td style="text-align: left;width: 25%;">'. htmlentities("GHS ".number_format( $searchpayementssum, 2, ".", ",")) . '</td>
  </tr>
...
$pageTitle = "<strong>" . htmlentities( " " . "Expense Items " . strtoupper("")) . "</strong>";
$pdf->writeHTML($pageTitle, true, 0, true, 0, 'L');
...
//$getMembers = $member ->get_members_by_groups_debotr_creditor($_SESSION['debtor_creditors_dropdown']);
while ($row = mysqli_fetch_array($searchExpense)) {
    ...
    <tbody>
        <tr>
           ...
           <td style="width: 45%; text-align: left;">' . htmlentities($row["acc_name"]) . '</td> 
            <td style="width: 35%; text-align: left">' . htmlentities("".number_format( $row["acc_amt"], 2, ".", ",")) . '</td>
 ...
 $tbl_head_prduct = '<table cellspacing = "0" cellpadding = "2" border = "1">';
   ...
   <tr>
<td style="text-align: left; width: 65%;"><strong>TOTAL AMOUNT</strong></td>     
 <td style="text-align: left; width: 35%;">'. htmlentities("GHS ".number_format( $searchexpensesum, 2, ".", ",")) . '</td>
  </tr>
