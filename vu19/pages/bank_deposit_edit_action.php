...
$activity = "Bank Deposit Edit";
$summary = "Bank Deposit Edit Activity Recorded From Device And IP"; 
 ...
 }else
{
  ...
  $stmtp = $pdo->prepare("UPDATE bank_deposits SET bank_name = :bank_name, amount_deposited =:amount_deposited,depositor =:depositor  WHERE u_id = :u_id AND id = :id");
 
  $stmtp ->bindParam(':bank_name', $bank_name,PDO::PARAM_STR);
  $stmtp ->bindParam(':amount_deposited', $acc_amt,PDO::PARAM_STR);
  $stmtp ->bindParam(':depositor', $depositor,PDO::PARAM_STR);
...
if($stmtp->execute()){
    ...
    //  if($secretary->saveMessageac()){
        header('Location:' . 'r?inv17ml44=bank_deposits');    
    //  }
