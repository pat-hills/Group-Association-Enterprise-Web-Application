...
$activity = "Bill Item Edit";
$summary = "Bill Item Edit Activity Recorded From Device And IP"; 
 ...
 }else
{
  ...
  $nameofprogram = filter_input(INPUT_POST, 'billname', FILTER_SANITIZE_STRING,FILTER_SANITIZE_SPECIAL_CHARS);
  $nameofprogram = preg_replace('/\s+/', '-', $nameofprogram);

  $stmtp = $pdo->prepare("UPDATE bill_item SET bill_name = :name_of_program  WHERE u_id = :u_id AND id = :id");
...
if($stmtp->execute()){
    ...
    //  if($secretary->saveMessageac()){
        header('Location:' . 'r?inv17ml44=add_bill_items');    
    //  }
