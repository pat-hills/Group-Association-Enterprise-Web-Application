...
$activity = "Bank Details Edit";
$summary = "Bank Details Edit Activity Recorded From Device And IP"; 
 ...
 }else
{
  ...
  $acc_number = filter_input(INPUT_POST, 'acc_number', FILTER_SANITIZE_STRING,FILTER_SANITIZE_SPECIAL_CHARS);
  $stmtp = $pdo->prepare("UPDATE bank_details SET bank_name = :bank_name, acc_number =:acc_number,branch =:branch  WHERE u_id = :u_id AND id = :id");
 
  $stmtp ->bindParam(':bank_name', $bank_name,PDO::PARAM_STR);
  $stmtp ->bindParam(':acc_number', $acc_number,PDO::PARAM_STR);
  $stmtp ->bindParam(':branch', $branch,PDO::PARAM_STR);
...
if($stmtp->execute()){
    ...
    //  if($secretary->saveMessageac()){
        header('Location:' . 'r?inv17ml44=add_bank_details');    
    //  }
